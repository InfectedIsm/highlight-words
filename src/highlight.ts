...
class Highlight {
    constructor() {
        this.words = [];
        this.decorators = [];
        this.treeProvider = new tree_1.default(this.getWords());
        this.ranges = {};
        vscode_1.window.registerTreeDataProvider('hilightWordsExplore', this.treeProvider);
    }
    ...
    updateDecorations(active) {
        vscode_1.window.visibleTextEditors.forEach(editor => {
            if (active && editor.document != vscode_1.window.activeTextEditor.document)
                return;
            const text = editor.document.getText();
            
            // Regex to find all specified comments
            const commentRegex = /(\/\/.*)|(#.*)|(:.*)|(\/\*\*[\s\S]*?\*\/)/g;
            const commentRanges = [];
            let commentMatch;
            while ((commentMatch = commentRegex.exec(text)) !== null) {
                const startPos = editor.document.positionAt(commentMatch.index);
                const endPos = editor.document.positionAt(commentMatch.index + commentMatch[0].length);
                commentRanges.push(new vscode_1.Range(startPos, endPos));
            }
            ...
            this.words.forEach((w, n) => {
                ...
                while (match = regEx.exec(text)) {
                    ...
                    const matchRange = new vscode_1.Range(startPos, endPos);

                    // Check if the match is inside a comment range
                    const inComment = commentRanges.some(commentRange => commentRange.contains(matchRange));

                    if (!inComment) {
                        const decoration = { range: matchRange };
    ...
    updateOptions(word) {
        vscode_1.window.showQuickPick(["default"].concat(qpOptions)).then(option => {
            if (!option)
                return;
    ...
    addSelected(withOptions) {
        ...
        if (!word) {
            vscode_1.window.showInformationMessage('Nothing selected!');
            return;
        }
        ...
        if (!highlights || !highlights.length) {
            if (withOptions) {
                vscode_1.window.showQuickPick(qpOptions).then(option => {
                    if (!option)
                        return;
                    this.words.push({
                        expression: word,
    ...
    addRegExp(word) {
        try {
            ...
            vscode_1.window.showInformationMessage(word + ' is an invalid expression');
        }
    }
}
